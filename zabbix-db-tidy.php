...
try
{
   ...
   foreach ($queries as $idx => $query) {
         mylogger(sprintf("[%s] (%s / %s) %s", date('Y-m-d H:i:s'), $idx, count($queries), $query));
    ...
    $mail->AddStringAttachment(implode($lines, PHP_EOL),"output-".time().".txt", "quoted-printable", "text/plain");
